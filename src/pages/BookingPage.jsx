...
import { LEVELS, CONTACT } from "../data/constants";
  ...
  return (
    <Card style={{ maxWidth:580, margin:"0 auto", textAlign:"center", padding:"clamp(20px,5vw,36px) clamp(16px,5vw,28px)" }}>
      ...
      <p style={{ color:C.muted, fontSize:15, lineHeight:1.8, marginBottom:22, fontWeight:600 }}>
        ...
        on <strong>{desiredDate ? formatDate(desiredDate) : slot}</strong> is confirmed.
      </p>
      ...
      <div style={{
        ...
        <ul style={{ margin:0, paddingLeft:18, fontSize:13, color:"#166534", fontWeight:600, lineHeight:1.9 }}>
          ...
          <li>Our admin team will create your <strong>Google Meet session</strong> and email you the link before your class.</li>
          ...
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 14px" }}>
              <Field label="Card Number" error={errs.num}>
                <input value={card.num} onChange={e => setCard({ ...card, num:e.target.value })} placeholder="1234 5678 9012 3456" maxLength={19} style={iStyle} />
              </Field>
              <Field label="Name on Card" error={errs.cname}>
                <input value={card.name} onChange={e => setCard({ ...card, name:e.target.value })} placeholder="As shown on card" style={iStyle} />
              </Field>
              <Field label="Expiry Date" error={errs.exp} half>
                <input value={card.exp} onChange={e => setCard({ ...card, exp:e.target.value })} placeholder="MM / YY" maxLength={7} style={iStyle} />
              </Field>
              <Field label="CVV" error={errs.cvv} half>
                <input value={card.cvv} onChange={e => setCard({ ...card, cvv:e.target.value })} placeholder="•••" maxLength={4} style={iStyle} />
              </Field>
            </div>
            ...
            <p style={{ textAlign:"center", fontSize:12, color:C.muted, marginTop:12, fontWeight:700 }}>
              🔒 256-bit SSL encrypted · Our admin will email your Google Meet link before your class
            </p>
          </>
        )}
